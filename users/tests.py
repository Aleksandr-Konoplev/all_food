...
class UsersViewsTests(TestCase):
    ...
    def test_register_view_creates_inactive_user_and_sends_email(self, mock_send_email):
        response = self.client.post(
            ...
            data={
                ...
                'password1': os.getenv('TEST_PASSWORD1'),
                'password2': os.getenv('TEST_PASSWORD1'),
    ...
    def test_password_reset_confirm_updates_password(self):
        ...
        response = self.client.post(
            response.url,
            data={
                'new_password1': os.getenv('TEST_PASSWORD2'),
                'new_password2': os.getenv('TEST_PASSWORD2'),
            },
        )

        self.assertRedirects(response, reverse('users:password-reset-complete'))
        self.user.refresh_from_db()
        self.assertTrue(self.user.check_password(os.getenv('TEST_PASSWORD2')))
