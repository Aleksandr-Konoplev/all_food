...
class Table(models.Model):
    ...
    num_table = models.PositiveIntegerField(
        verbose_name="Номер столика", unique=True, validators=[MaxValueValidator(6)]
    )
    ...
    min_deposit = models.PositiveIntegerField(
        verbose_name="Минимальная сумма депозита", validators=[MaxValueValidator(50000)]
    )
    ...
    def clean(self):
        ...
        if self.table and self.start_at and self.end_at:
            ...
            if overlapping.exists():
                raise ValidationError({"table": "Этот столик уже забронирован на выбранное время."})
        # ---------------------------------------------------------------------------------------------------
